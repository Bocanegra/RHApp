...
@interface LASimpleCoreDataStack ()
...
@implementation LASimpleCoreDataStack
...
- (NSPersistentStoreCoordinator *)storeCoordinator {
    if (_storeCoordinator == nil) {
        ...
        if (![_storeCoordinator addPersistentStoreWithType:NSSQLiteStoreType
            ...
            NSNotification *note = [NSNotification notificationWithName:[LASimpleCoreDataStack persistentStoreCoordinatorErrorNotificationName]
                                                                 object:self
                                                               userInfo:@{@"error": err}];
...
+ (LASimpleCoreDataStack *)coreDataStackWithModelName:(NSString *)aModelName
...
+ (LASimpleCoreDataStack *)coreDataStackWithModelName:(NSString *)aModelName databaseURL:(NSURL *)aDBURL {
...
- (void)saveWithErrorBlock:(void(^)(NSError *error))errorBlock {
    ...
    if (!_context) {
        err = [NSError errorWithDomain:@"LASimpleCoreDataStack"
                                  code:1
                              userInfo:@{NSLocalizedDescriptionKey :
                                             @"Attempted to save a nil NSManagedObjectContext. This LASimpleCoreDataStack has no context - probably there was an earlier error trying to access the CoreData database file."}];
        errorBlock(err);
...
- (NSArray *)executeRequest:(NSFetchRequest *)request
    ...
    if (!_context) {
        err = [NSError errorWithDomain:@"LASimpleCoreDataStack"
                                  code:1
                              userInfo:@{NSLocalizedDescriptionKey :
                                             @"Attempted to search a nil NSManagedObjectContext. This LASimpleCoreDataStack has no context - probably there was an earlier error trying to access the CoreData database file."}];
        errorBlock(err);
    } else {
